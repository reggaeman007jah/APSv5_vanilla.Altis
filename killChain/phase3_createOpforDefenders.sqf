...
instead of using patrol points taken, use mission time as a more linear and scalable measure 
the longer the mision goes on, the nastier the defences become 
...
for "_i" from 1 to _rndOp1 do {
	...
	_pos = [RGG_patrol_obj, 0, 100] call BIS_fnc_findSafePos;
...
// redirects any indifor units incorrectly sent to the old point as part of an RF action 
execVM "killChain\systems\insuranceSystems\phase3Timer.sqf";
...
while {RFCHECK} do {
	...
	if ((RGG_redzoneEast <= 3) && (RGG_redzoneIndi >=3)) then { 
		...
		if (_coreIndi > 4) then {
			...
			execVM "killChain\phase4_createOpforRF.sqf";
			RFCHECK = false;		
		};
	};
