...
Actions:
	...
	add a core check to make sure progression does not happen until some indifor are actually at the point 
	consider thinking about different mission states, and having checks (with global outputs) happen relating to the mission 
	so you could make your checks specific to the thing you are interested in 
	what do we care about in eah stage of the mission? We should only check for these things 
	We must reduce counting wherever we can !!!!
...
_RGG_reinforcementTrigger = 8; 

// sets patrol phase timer and creates basic move orders for opfor, to prevent stalemate 
execVM "killChain\systems\insuranceSystems\phase5Timer.sqf";
