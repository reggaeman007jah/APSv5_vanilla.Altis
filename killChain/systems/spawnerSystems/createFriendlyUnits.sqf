/*
From: killchain/killChainInit.sqf 

Receives: position value RGG_initStartPos, also uses global objective value (although not given as a direct arg)

Actions:
Create muster points for each fireteam, so they get into close position before heading off 
convert spawnedIndiUnit to RGG_totalSpawnedIndiUnits
...
systemChat "killChain\systems\spawnerSystems\createFriendlyUnits.sqf";

_spawnPos = _this select 0; // RGG_initStartPos 
// to do - have a local obj pos sent in here 
...
for "_i" from 1 to _numberOfCycles do {
	...
	spawnedIndiUnit = spawnedIndiUnit +5; // used for stats 
};
